...
def get_vdu_servicemonitoring(template):

    monitoring_dict = dict()
    policy_dict = dict()
    policy_dict['vdus'] = collections.OrderedDict()
    vdunode= [node for node in template['topology_template']['node_templates'] if fnmatchcase(node,'VDU*')]
    ...
    for vdu in vdunode:
            if 'service_monitoring_policy' in template['topology_template']['node_templates'][vdu]['properties']:
                ...
                policy_dict['vdus'][vdu]=mon_policy
                del template['topology_template']['node_templates'][vdu]['properties']['service_monitoring_policy']
    if policy_dict.get('vdus'):
            monitoring_dict = policy_dict
    return monitoring_dict,template
