...
class VNFServiceMonitor(object):
    ...
    OPTS = [
        cfg.ListOpt(
            'service_monitor_driver', default=['zabbix'],
            help=_('Service monitoring driver to communicate with '
                   'Hosting VNF/logical service '
                   'instance tacker plugin will use')),
    ]
    cfg.CONF.register_opts(OPTS, 'tacker')
    ...
    def _create_service_monitoring_dict(self,dev_attrs, mgmt_url):
        ...
        vdunode = [node for node in svcmonitoring_dict['vdus'] if fnmatchcase(node, 'VDU*')]
        for vdu in vdunode:
            temp = ast.literal_eval(mgmt_url)
            svcmonitoring_dict['vdus'][vdu]['mgmt_ip'] = temp[vdu]
        return svcmonitoring_dict
